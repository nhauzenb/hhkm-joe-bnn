...
NUTS_one_step <- function(theta, iter, iterMax, f, grad_f, f_list, par_list, delta = 0.5, max_treedepth = 10, eps = 1, verbose = TRUE){
  ...
  logu <- f(theta = theta, f_list = f_list) - 0.5 * sum(r0^2/M_diag) + log(runif(1))
  
  if(is.nan(logu)){
    warning("NUTS: sampled slice log(u) is NaN")
    logu <- log(runif(1, 0, 1e5))
  }
  theta_minus <- theta
  theta_plus <- theta
  r_minus <- r0
  ...
  while(s == 1){
    ...
    if(direction == -1){
      temp <- build_tree(theta_minus, r_minus, logu, direction, j, eps, theta, r0, f, grad_f, f_list, M_diag)
      theta_minus <- temp$theta_minus
      r_minus <- temp$r_minus
    } else{
      temp <- build_tree(theta_plus, r_plus, logu, direction, j, eps, theta, r0, f, grad_f, f_list, M_diag)
...
build_tree = function(theta, r, logu, v, j, eps, theta0, r0, f, grad_f, f_list, M_diag, Delta_max = 1000){
  if(j == 0){
    ...
    obj0 <- build_tree(theta, r, logu, v, j-1, eps, theta0, r0, f, grad_f, f_list, M_diag)
    ...
    if(obj0$s == 1){
      if(v == -1){
        obj1 <- build_tree(obj0$theta_minus, obj0$r_minus, logu, v, j-1, eps, theta0, r0, f, grad_f, f_list, M_diag)
        theta_minus <- obj1$theta_minus
        r_minus <- obj1$r_minus
      } else{
        obj1 <- build_tree(obj0$theta_plus, obj0$r_plus, logu, v, j-1, eps, theta0, r0, f, grad_f, f_list, M_diag)
...
hmc_deep <- function (theta, f, grad_f, f_list, epsilon = .1, L = 10, acc = 0) {
  ...
  p_tilde <- -p_tilde
  # Evaluate log posterior and kinetic energy at start and end of trajectory
  ...
  # Accept or reject the state at end of trajectory (isTRUE guards against NaN)
...
get.post.grad_k <- function(theta, f_list = f_list){
  # Gradient of the log posterior w.r.t. theta (kappa column nr2, layer nr1)
  # Unpack list of arguments
  k_draw    <- f_list$k_draw.nr
  k.V       <- f_list$k.V
  nr1       <- f_list$nr1
  ...
  for (nn in 1:Q){
    Mlay <- MM[nn]
    z.nn <- X.hat.nr[,1:Mlay,nn]%*%as.matrix(k_draw[1:Mlay,,nn])
  ...
  dhqdkq <- as.numeric(b_draw.nr)*chain.grad*acf_set[[acf_draw[nr1]]][["grad"]](X.hat.nr[,1:MM[nr1],nr1]%*%as.matrix(theta))*normalizer # Inner derivative of act. function 
